...
void main() {
    ...
    fragColor.a = height; // Set alpha based on height for transparency
}
`;

export default function Aurora(props: AuroraProps) {
    
  const { colorStops = ["#00d8ff", "#0b0d12", "#0b0d12"], amplitude = 1.0 } =
    props;
  ...
  useEffect(() => {
    ...
    let animateId = 0;
    const speedVariation = (Math.random() - 0.5) * 2; // مقدار تصادفی برای تغییر جهت حرکت
  
    const update = (t: number) => {
      animateId = requestAnimationFrame(update);
  
      const { time = t * 0.01, speed = 1.0 } = propsRef.current;
      program.uniforms.uTime.value = (time * speed * 0.1) * speedVariation;
